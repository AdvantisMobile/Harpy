...
export class UserService {
  ...
  getClient(key: string) {
    return this.db.object<Client>(`clients/${key}`).valueChanges();
  }

  getAllClient() {
    return mapToModel(this.db.list<Client>("/clients").snapshotChanges());
  }

  updateClient(client: Client, key: string) {
    this.db.object(`clients/${key}`).update(client);
  }

  deleteClient(key: string) {
    this.db.object(`clients/${key}`).remove();
  }
}
